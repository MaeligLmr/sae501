...
namespace DialogueEditor
{
    public class DialogueEditorWindow : EditorWindow
    {
        ...
        private const string HELP_URL = "https://grasshopdev.github.io/dialogueeditor.html";
                    ...
                    if (selectedNode is UISpeechNode)
                    {
                        ...
                        if(true)
                        {
                            ...
                            if (node.AdvanceDialogueAutomatically)
                            {
                                ...
                                if(node.Audio != null){
                                    node.WaitForAudioEnd = EditorGUILayout.Toggle("Wait until audio finished", node.WaitForAudioEnd);
                                }
                            }
                            EditorGUILayout.Space();
                        }
        ...
        private void AddNewSpeechNode()
        {
            ...
            UISpeechNode uiNode = new UISpeechNode(newSpeech, new Vector2(position.width / 2, position.height / 2));
            uiNodes.Add(uiNode);

            SelectNode(uiNode, true);
            Repaint();
        }
